...
session = cluster.connect('cmp_qa_stargate')
session.row_factory = pandas_factory
session.default_fetch_size = None

# queries
# NOTE: This driver does not support COPY. USE cqlsh -u devadmin -p Keys2TheK1ngd0m 172.30.100.206
cass_select_query = 'SELECT * FROM "provisioning_request";'

# execution
rows = session.execute(cass_select_query, timeout=None)
my_list = []
my_list.extend(rows._current_rows.values.tolist())
